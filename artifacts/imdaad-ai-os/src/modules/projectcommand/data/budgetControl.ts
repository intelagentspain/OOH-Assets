...
const vendorByPackage: Record<string, string[]> = {
  ...
  substructure: ['Arabian Waterproofing', 'Al Habtoor Engineering'],
  superstructure: ['Al Habtoor Engineering'],
  facade: ['Emirates Glass'],
  mep: ['Voltas MEP'],
  fitout: ['Fit-out Contractor'],
  testing: ['Commissioning Authority'],
  handover: ['Handover Team'],
};
...
function buildFromProject(project: ProjectCommandProject): BudgetControlData {
  ...
  const packages = buildPackages(project, {
    substructure: { change: 4.2, committed: contractM * 0.16, actual: contractM * 0.14, forecast: contractM * 0.18, risk: highPressure ? 'critical' : 'high', delay: project.id.includes('verdana') ? 0 : 8, exposure: project.id.includes('verdana') ? 0.8 : 11.8, vendorScore: 72, claims: 4.8, disputed: 1.6 },
    superstructure: { change: project.id.includes('bayz') ? 1.1 : 0.8, committed: contractM * 0.26, actual: contractM * 0.2, forecast: contractM * (project.id.includes('bayz') ? 0.33 : 0.28), risk: highPressure ? 'high' : 'medium', delay: project.id.includes('bayz') ? 14 : 3, exposure: project.id.includes('bayz') ? 18.6 : 3.2, vendorScore: 78, claims: 7.4, disputed: 2.2 },
    facade: { change: project.id.includes('bayz') ? 4.8 : 0.4, committed: contractM * 0.11, actual: contractM * 0.04, forecast: contractM * (project.id.includes('bayz') ? 0.18 : 0.14), risk: project.id.includes('bayz') ? 'high' : 'medium', delay: project.id.includes('bayz') ? 21 : 2, exposure: project.id.includes('bayz') ? 13.7 : 1.4, vendorScore: 69, claims: 4.8, disputed: 3.1 },
    mep: { change: 1.7, committed: contractM * 0.17, actual: contractM * 0.08, forecast: contractM * 0.19, risk: 'high', delay: project.id.includes('verdana') ? 0 : 5, exposure: project.id.includes('verdana') ? 0.7 : 6.4, vendorScore: 76, claims: 2.6, disputed: 0.9 },
  });
  ...
  const contingency = Math.round(project.contractValue * (project.id.includes('bayz') ? 0.08 : 0.07));
  ...
  return {
    ...
    variations: [
      { id: 'VO-31', projectId: project.id, packageId: 'facade', title: 'Facade bracket redesign - high zone', vendorName: 'Emirates Glass', amount: m(project.id.includes('bayz') ? 4.8 : 1.2), status: 'Under Review', reason: 'Design coordination change', submittedBy: 'Facade Contractor', submittedDate: '2026-04-28', approvalImpact: '+ revised budget', programmeImpact: 'Avoids 9 days of envelope delay', approvalOwner: 'Commercial Manager', decisionDue: '2026-05-10', linkedRiskId: 'r-facade' },
      { id: 'VO-32', projectId: project.id, packageId: 'superstructure', title: 'Additional crane overtime allowance', vendorName: project.mainContractor, amount: m(project.id.includes('bayz') ? 2.9 : 0.8), status: 'Pending Client Approval', reason: 'Acceleration and logistics recovery', submittedBy: project.mainContractor, submittedDate: '2026-05-01', approvalImpact: '+ pending exposure', programmeImpact: 'Recovers 4-7 days', approvalOwner: 'Project Director', decisionDue: '2026-05-12', linkedRiskId: 'r-crane' },
      { id: 'VO-33', projectId: project.id, packageId: 'mep', title: 'Podium MEP coordination package', vendorName: 'Voltas MEP', amount: m(1.7), status: 'Approved', reason: 'Clash resolution', submittedBy: 'MEP Contractor', submittedDate: '2026-04-18', approvalImpact: '+ approved changes', programmeImpact: 'Protects rough-in sequence', approvalOwner: 'Commercial Manager', decisionDue: '2026-04-26', linkedRiskId: 'r-mep' },
    ...
    risks: [
      { id: 'CR-01', packageId: 'substructure', title: 'Waterproofing rework growth', exposure: m(project.id.includes('verdana') ? 0.7 : 6.2), probability: 0.62, expectedImpact: m(project.id.includes('verdana') ? 0.4 : 3.8), mitigation: 'Second crew, inspection hold points, revised method statement.', status: 'mitigating' },
      { id: 'CR-02', packageId: 'facade', title: 'Facade procurement variance', exposure: m(project.id.includes('bayz') ? 6.9 : 1.1), probability: 0.48, expectedImpact: m(project.id.includes('bayz') ? 3.3 : 0.5), mitigation: 'Lock release dates and vendor clarification before next package gate.', status: 'open' },
    ...
    evm: { pv: project.plannedValue, ev: project.earnedValue, ac: project.actualCost, cpi: project.cpi, spi: project.spi, eac: project.forecastCost, tcpi: project.id.includes('bayz') ? 1.08 : project.id.includes('verdana') ? 0.96 : 0.94, costVariance: project.costVariance },
    ...
    missingDataWarnings: project.id.includes('bayz')
    ...
    aiNarrative: `${project.name} is forecast at AED ${Math.round(project.forecastCost / 1_000_000)}M against an approved budget of AED ${Math.round(project.contractValue / 1_000_000)}M. The main drivers are ${project.id.includes('bayz') ? 'steel escalation, crane productivity loss, and facade procurement variance' : project.id.includes('verdana') ? 'commissioning allowances, authority re-test provisions, and final snagging labour' : 'waterproofing rework, concrete escalation, and MEP coordination exposure'}. CPI is ${project.cpi.toFixed(2)}, so AI recommends protecting contingency, closing pending variations, and challenging vendor claims before the next payment cycle.`,
    managerActions: [
